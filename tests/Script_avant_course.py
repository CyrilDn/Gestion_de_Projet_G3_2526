...
class ScriptAvantCourse:
    ...
    def _verifier_capteurs(self):
        ...
        try:
            ...
            for i in range(3):
                distance = (
                    self.controleur.capteur_ultrason.mesurer_distance()
                    if self.controleur.capteur_ultrason
                    else None
                )
                print(f"  - Mesure de distance {i + 1}: {distance} cm")
                self.data.ajouter_log_info(
                    f"Ultrason mesure {i + 1} : {distance:.2f} cm"
                )
        except Exception as e:
            self.data.ajouter_log_erreur(f"Ultrason mesure {i + 1} : {e}")
        time.sleep(0.5)

    def _verifier_moteurs1(self):
        """Vérifie uniquement le servo"""
        try:
            print("test Desmet")
            print("[*] 2. Vérification du servo...")
            ...
            self.data.ajouter_log_erreur(f"Servo : {e}")
    ...
    def _verifier_batterie(self):
        ...
        try:
            ...
            niveau = (
                self.controleur.telemetrie.lire_tension()
                if self.controleur.telemetrie
                else None
            )
            courant = (
                self.controleur.telemetrie.lire_courant()
                if self.controleur.telemetrie
                else None
            )

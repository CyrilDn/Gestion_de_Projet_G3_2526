...
class ControleurVoiture:
    ...
    def __init__(self):
        ...
        self.dernier_passage_arrivee = 0
    ...
    def _initialiser_composants(self):
        ...
        try:
            ...
            self.servo = ServoDirectionPCA(canal=0, pca=self.pca)

            # Initialiser les capteurs
            self.capteur_ultrason1 = CapteurUltrason(
                pin_trigger=6, pin_echo=5
            )  # devant
            self.capteur_ultrason2 = CapteurUltrason(
                pin_trigger=26, pin_echo=19
            )  # droite
            self.capteur_ultrason3 = CapteurUltrason(
                pin_trigger=11, pin_echo=9
            )  # gauche
    ...
    def run(self, nombre_tour=3, resume=False):
        ...
        try:
            ...
            while True:
                ...
                try:
                    distance1 = (
                        self.capteur_ultrason1.mesurer_distance()
                        if self.capteur_ultrason1
                        else None
                    )
                ...
                try:
                    distance2 = (
                        self.capteur_ultrason2.mesurer_distance()
                        if self.capteur_ultrason2
                        else None
                    )
                except (TimeoutError, ValueError):
                ...
                try:
                    distance3 = (
                        self.capteur_ultrason3.mesurer_distance()
                        if self.capteur_ultrason3
                        else None
                    )
                except (TimeoutError, ValueError):
                ...
                if tension is not None and courant is not None:
                    print(
                        f"[📊] Télémétrie - Tension: {tension:.2f} V, Courant: {courant:.2f} mA"
                    )
                ...
                if arrivee_detectee:
                    ...
                    if maintenant - self.dernier_passage_arrivee > 2:
                        self.dernier_passage_arrivee = maintenant
                        self.compteur_tours += 1
                        print(
                            f"Passage ligne arrivée — tour {self.compteur_tours}/{nombre_tour}"
                        )
                        self.data.ajouter_log_info(
                ...
                if not self.en_marche:
                    rouge, vert, bleu, clair = (
                        self.capteur_couleur.lire_valeurs_brutes()
                        if self.capteur_couleur
                        else (0, 0, 0, 0)
                    )
                    couleur_dominante = (
                        self.capteur_couleur.detecter_couleur_dominante(
                            rouge, vert, bleu, clair
                        )
                        if self.capteur_couleur
                        else "inconnu"
                    ...
                    print(
                        f"[🎨] Capteur Couleur - R: {rouge}, G: {vert}, B: {bleu}, C: {clair}"
                    )
                    print(
                        f"[🎨] Capteur Couleur - Couleur dominante: {couleur_dominante}"
                    )
                    self.data.ajouter_log_info(
                    ...
                    if not self.gestion_securite.verifier_securite_feu(
                        couleur_dominante
                    ):
                        self.gestion_securite.arreter_urgence()
                        print(
                            "[🛑] Arrêt d'urgence déclenché en raison du feu de signalisation!"
                        )
                        self.data.ajouter_log_erreur(
                            "Arrêt d'urgence déclenché (feu/capteur couleur)"
                        )
                        break
                    ...
                    if couleur_dominante == "vert":
                        ...
                        self.data.ajouter_log_info(
                            "Feu vert détecté - démarrage de la course"
                        )
                        self.en_marche = True
                    else:
                        # En attente du feu vert
                        print(
                            f"[🔴] En attente du feu vert (capteur: {couleur_dominante})"
                        )
                        self.moteur1.arreter()
                        self.moteur2.arreter()
                        time.sleep(0.1)
